...
const messageViewModel = (session, thread: thread) => {
  ...
  const cache = messageCache(session);

  session.socket
    .on('createmessage', (data) => {
      const message = makeObject(data.message)[0];

      cache.removeRead(message.createdbyid);
      cache.add(message);
    ...
    .on('deletemessage', (data) => cache.delete(makeObject(data.message)[0]))
    .on('updatemessage', (data) => cache.update(makeObject(data.message)[0]))
    .on('react', (data) => {
      cache.react(makeObject(data.reaction)[0]);
    })
    .on('read', (data) => {
      const user = makeObject(data.user);

      cache.removeRead(user.userid);
      cache.addRead(data.messageid, user);
    });
